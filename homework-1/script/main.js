window.onscroll = function showHeader () {
    ...
    const subNuv = document.querySelector('.sub-nuv');
    ...
    if(window.pageYOffset > (header.clientHeight - (subNuv.clientHeight / 2))) {
        subNuv.classList.add('sub-nuv-fixed');
    } else {
        subNuv.classList.remove('sub-nuv-fixed');
    }
}
...
const subMenuLink = document.querySelectorAll('.sub-nuv-link');

subMenuLink.forEach((elem) => {
    elem.addEventListener('click', function () {
        subMenuLink.forEach(value => value.classList.remove('active'));
